...
int ACMEObject::setSignature( const BIN *pPri,const QString strHash )
{
    ...
    if( ret == JSR_OK )
    {
        manApplet->log( QString( "== Compute Signature =="));
        manApplet->log( QString( "JWS Hash      : %1").arg( strHash ));
        manApplet->log( QString( "JWS Source    : %1").arg( getHexString( &binSrc )));
        manApplet->log( QString( "JWS Signature : %1").arg( getHexString( &binSign )));
...
int ACMEObject::verifySignature( const BIN *pPub )
{
    ...
    manApplet->log( QString( "== Verify Signature =="));
    manApplet->log( QString( "JWS Hash      : %1").arg( strHash ));
    manApplet->log( QString( "JWS Source    : %1").arg( getHexString( &binSrc )));
    manApplet->log( QString( "JWS Signature : %1").arg( getHexString( &binSign )));
