...
void LCNInfoDlg::initialize()
{
    ...
    if( manApplet->isLicense() )
    {
        ...
        ret = JS_LCN_IsValid( &sLicenseInfo, LICENSE_PRODUCT_CERTMAN_NAME, sLicenseInfo.sSID, time(NULL) );
...
bool LCNInfoDlg::isValidLCN( const BIN *pLCN, const char *pSID )
{
    ...
    ret = JS_LCN_IsValid( &sInfo, LICENSE_PRODUCT_CERTMAN_NAME, pSID, time(NULL) );
...
void LCNInfoDlg::clickGet()
{
    ...
    {
        QString strFile = findFile( this, JS_FILE_TYPE_CERT, manApplet->curFolder() );
    ...
    ret = JS_LCN_IsValid( &sInfo, LICENSE_PRODUCT_CERTMAN_NAME, sInfo.sSID, time(NULL) );
    if( ret != LICENSE_VALID )
    {
