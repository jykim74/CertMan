...
void LCNInfoDlg::clickGet()
{
    ...
    {
        QString strFile = manApplet->findFile( this, JS_FILE_TYPE_LCN, "" );
...
void LCNInfoDlg::clickFree()
{
    ...
    {
        manApplet->warningBox( tr( "An email address is required to issue a free license." ), this );
        mEmailText->setFocus();
        return;
    }
    ...
    if( ret == 0 )
    {
        if( manApplet->yesOrNoBox(tr("A free license has been issued. Restart to apply it?"), this, true))
            manApplet->restartApp();
